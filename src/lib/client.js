    ...
    class Client extends EventEmitter
    {
        ...
        _connect(address, options)
        {
            ...
            this.socket.on("message", async (message) =>
            {
                ...
                if (message.method && this.rpc_methods[message.method])
                {
                    let response = null
                    try
                    {
                        response = await this.rpc_methods[message.method](message.params)
                    }
                    catch (error)
                    {
            ...
            {
                if (this.ready) this.emit("close", code, message)
